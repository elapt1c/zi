[![unittests](https://github.com/robertdavidgraham/zorpinvader/actions/workflows/unittests.yml/badge.svg?branch=master)](https://github.com/robertdavidgraham/zorpinvader/actions/workflows/unittests.yml/?branch=master)

# ZORPINVADER: Mass IP port scanner
...
NOTE: zorpinvader uses its own **ad hoc TCP/IP stack**. Anything other than
...
use `--src-port` to configure which source ports zorpinvader uses, then also
...
Bitcoin wallet address: 1ZORPINVADERaHUiyTtR3bJ2sLGuMw5kDBaj4T
...
or `clang`).
	...
	git clone https://github.com/robertdavidgraham/zorpinvader
	cd zorpinvader
	make

This puts the program in the `zorpinvader/bin` subdirectory. 
...
systems (Windows, macOS, etc.). Here's some additional build info:
  ...
  * FreeBSD: type `gmake`
  * other: try just compiling all the files together, `cc src/*.c -o bin/zorpinvader`
...
Usage is similar to `nmap`. To scan a network segment for some ports:
	...
	# zorpinvader -p80,8000-8100 10.0.0.0/8 2603:3001:2d00:da00::/112
...
into the program:
	...
	# zorpinvader -p80,8000-8100 10.0.0.0/8 2603:3001:2d00:da00::/112 --echo > xxx.conf
	# zorpinvader -c xxx.conf --rate 1000


## Banner checking

ZorpInvader can do more than just detect whether ports are open. It can also
complete the TCP connection and interaction with the application at that
port in order to grab simple "banner" information.

ZorpInvader supports banner checking on the following protocols:
  * FTP
  * HTTP
  * IMAP4
  * memcached
...
The problem with this is that zorpinvader contains its own TCP/IP stack
...
the connection before zorpinvader can grab the banner.

The easiest way to prevent this is to assign zorpinvader a separate IP
address. This would look like one of the following examples:

	# zorpinvader 10.0.0.0/8 -p80 --banners --source-ip 192.168.1.200
      # zorpinvader 2a00:1450:4007:810::/112 -p80 --banners --source-ip 2603:3001:2d00:da00:91d7:b54:b498:859d

The address you choose has to be on the local subnet and not otherwise
be used by another system. ZorpInvader will warn you that you've made a
...
firewall the port that zorpinvader uses. This prevents the local TCP/IP stack
from seeing the packet, but zorpinvader still sees it since it bypasses the
local stack. For Linux, this would look like:

	# iptables -A INPUT -p tcp --dport 61000 -j DROP
	# zorpinvader 10.0.0.0/8 -p80 --banners --source-port 61000
...
On FreeBSD and older MacOS, use an `ipfw` command: 
	...
	# zorpinvader 10.0.0.0/8 -p80 --banners --source-port 40000
...
are necessary. However, zorpinvader is still designed to work best using its
...
with the entire Internet in mind. It might look something like this:
	...
	# zorpinvader 0.0.0.0/0 -p0-65535
...
you want to use the following syntax:
	...
	# zorpinvader 0.0.0.0/0 -p0-65535 --excludefile exclude.txt
...
saved to a file instead. Therefore, you want something like:
	...
	# zorpinvader 0.0.0.0/0 -p0-65535 -oX scan.xml
...
take forever to scan the Internet. You need to speed it up as so:
	...
	# zorpinvader 0.0.0.0/0 -p0-65535 --max-rate 100000
...
To use this configuration file, use the `-c`:
	...
	# zorpinvader -c myscan.conf

This also makes things easier when you repeat a scan.

By default, zorpinvader first loads the configuration file 
`/etc/zorpinvader/zorpinvader.conf`. Any later configuration parameters override what's
...
By default, zorpinvader produces fairly large text files, but it's easy 
...
2. binary: This is the zorpinvader builtin format. It produces much smaller files so that
...
to `nmap` users, even though it's fundamentally different. ZorpInvader is tuned
...
You can think of `zorpinvader` as having the following settings permanently
enabled:
...
crash under the load that zorpinvader can generate.
...
A key difference between ZorpInvader and other scanners is the way it randomizes
targets.
...
ZorpInvader supports not only IP address ranges, but also port ranges. This means
...
ZorpInvader is designed for the next level of scalability, the "C10M problem".

The C10M solution is to bypass the kernel. There are three primary kernel
bypasses in ZorpInvader:
* custom network driver
* user-mode TCP stack
* user-mode synchronization

ZorpInvader can use the PF_RING DNA driver. This driver DMAs packets directly
...
ZorpInvader has its own built-in TCP stack for grabbing banners from TCP
...
ZorpInvader has no "mutex". Modern mutexes (aka. futexes) are mostly user-mode,
...
Instead, ZorpInvader uses "rings" to synchronize things, such as when the
...
ZorpInvader supports IPv6, but there is no special mode, both are supported
at the same time. (There is no `-6` option -- it's always available).

In any example you see of zorpinvader usage,
...
extra seconds to be read on startup (zorpinvader sorts the addresses and removes
duplicates before scanning).

Remember that zorpinvader contains its own network stack. Thus, the local machine
you run zorpinvader from does not need to be IPv6 enabled -- though the local
...
adapter and a special driver known as ["PF_RING ZC" from ntop](https://www.ntop.org/installation-guide-for-pf_ring/). ZorpInvader doesn't need to be rebuilt in order to use PF_RING. To use PF_RING,
...
When ZorpInvader detects that an adapter is named something like `zc:enp1s0` instead
...
to avoid overloading your local router:
    ...
    $ bin/zorpinvader 0.0.0.0/4 -p80 --rate 100000000 --router-mac 66-55-44-33-22-11
...
without the transmit overhead:
    ...
    $ bin/zorpinvader 0.0.0.0/4 -p80 --rate 100000000 --offline
...
at which they perform this calculation, making `zorpinvader` much faster.
